<?php
class Model_menuadmin extends CI_Model
{
	...
	public function init_view()
	{
	   $data['list_menu']=  $this->menu_parent();   
        $data['module_cb'] = $this->model_public->QueryMultyValue("SELECT * from _module");   
	...
	private function menu_parent()
	{
	    ...
	    if(!empty($query))
	    {
            foreach($query as $row):  
    	        ...
    	        $link	    = $row->link;
                $aktif      = $row->aktif;
                $module     = $this->master->getData('module_name','_module','id_module',$row->id_module);
                ...
                $cek = $this->model_public->QueryNumRows("SELECT * from _menu_admin where parent_id='".$id_menu."'");
    	        if ($id == $id_menu && $cek > 0){
    	        ...
    	        $table_induk = $this->model_public->QuerySingleValue("SELECT count(*) as ct from _menu_admin where parent_id = '$id_menu'");
      ...
      //----------------------------------------------------------------------
	    ...
	    if(!empty($query))
	    {
            foreach($query as $row):	      
                ...
                $table_anak = $this->model_public->QuerySingleValue("SELECT count(*) as ct from _menu_admin where parent_id = '$id_menu'");
	...
	public function init_add_save()
	{
        ...
        $order_no  = $this->master->maxDataDetail('order_no','_menu_admin','parent_id',$parent_id); 
        ...
        $menu = $this->model_public->QuerySingleValue("SELECT max(id_menu) as id_max from _menu_admin");
        ...
        $this->db->trans_start();
        $this->db->insert('_menu_admin', $data);
	...
	public function init_edit_save()
	{
        ...
        $this->db->update('_menu_admin',$data); 
        $this->db->trans_complete();        
        ...
        {   
            ...
            $status_property['url_process'] = '';
            $this->model_public->messaga_status_user($status_property);
            redirect('menuadmin/view/'.$parent_id);
        }
	}
	...
	public function init_delete()
	{
        ...
        {
            //------------------ Hapus Gambar --------------------
            $this->db->delete('_menu_admin', array('id_menu' => $id_menu));
    ...
    public function init_edit($id_menu)
    {
        $menu = $this->model_public->QuerySingleValue("SELECT *
                                                      FROM _menu_admin
                                                      WHERE _menu_admin.id_menu = '$id_menu' "); 

        $data['data'] = $menu;

        $module = $this->model_public->QuerySingleValue("SELECT * FROM _module WHERE id_module='".$menu->id_module."'");
        if ($module){
            $data['id_module'] = $module->id_module;
            $data['module_name'] = $module->module_name;
            $data['module_cb'] = $this->model_public->QueryMultyValue("SELECT * from _module where id_module <> '".$menu->id_module."'"); 
        }
