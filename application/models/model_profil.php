...
class Model_profil extends CI_Model {
    ...
    {  
        ...
        $config['total_rows'] = $this->db->get('tb_profil')->num_rows();
    ...
    public function init_edit()
    {
        ...
        $profil =  $this->model_public->QuerySingleValue("SELECT * FROM tb_profil WHERE id='$id'"); 
    ...
    public function init_add_save($username)
    {
        ...
        $profil = $this->model_public->QuerySingleValue("SELECT max(id) as id_max from tb_profil");
        $id_profil = $profil->id_max + 1;    
        ...
        {   
            if ($_FILES['foto']['name']!=''){
                $id_kat_foto = '1';
                $uploadFoto = $this->master->uploadFoto('profil',$judul, $teks_foto, $id_kat_foto, $tabel, $id_profil, $aktif, $tag, $entri);
            }
    ...
    public function init_edit_save($username)
    {
        ...
        {   
            if ($_FILES['foto']['name']!=''){
                ...
                $id_kat_foto = '1';
                $this->master->uploadFoto('profil',$judul, $teks_foto, $id_kat_foto, $tabel, $id, $aktif, $tag, $edit);
            }
            else{
                ...
                if ($foto){
                    ...
                    $this->master->updateDataFoto('profil', $judul, $teks_foto, $id_kat_foto, $tabel, $id_foto, $aktif, $tag, $edit);   
                }
            }
    ...
    {
        //validate Checkbox
        $this->master->validateCheckbox('profil');
        ...
        if ($publish == 'publish'){
            ...
            for($i=0;$i<$jml_check;$i++){
                ...
                $this->db->update('tb_profil',array('aktif' => '1'));
                $this->db->trans_complete();
                if ($this->db->trans_status()==false)
                    $this->db->trans_rollback();
                else{   
                    $this->master->publishFotoContent('tb_profil',$check[$i]); 
                }   
            }
            $status_property['parameter'] = 'view';
        ...
        if ($unpublish == 'unpublish'){
            ...
            for($i=0;$i<$jml_check;$i++){
                $this->db->trans_start();
                $this->db->where('id', $check[$i]);
                $this->db->update('tb_profil',array('aktif' => '0'));
                $this->db->trans_complete();
                if ($this->db->trans_status()==false)
                    $this->db->trans_rollback();
                else{   
                    $this->master->unpublishFotoContent('tb_profil',$check[$i]); 
                }    
            }
            $status_property['parameter'] = 'view';
        ...
        if ($delete == 'delete'){
            ...
            for($i=0;$i<$jml_check;$i++){
                $this->master->deleteFoto('tb_profil',$check[$i]);  
            }
